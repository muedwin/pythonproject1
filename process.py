...
def log_transaction(transaction_type, date, stock, number_of_shares, price, fees, ledger_file):
    ...
    '''
    if log_transaction[0] == "buy":
        balance = -(log_transaction(price)*log_transaction(number_of_shares)+log_transaction(fees))
    else:
        balance = (log_transaction(price)*log_transaction(number_of_shares)-log_transaction(fees))
    x = log_transaction(transaction_type)
    y = log_transaction(date)
    z = log_transaction(stock)
    k = log_transaction(number_of_shares)
    j = round(log_transaction(price),2)
    sys.stdout = open ("ledger_file", "w")
    with open('ledger_file','w') as txt:
        print(str(x),str(y),str(z),str(k),str(j),str(balance))
